...
from core.taggit.forms import TagField
from core.taggit.models import TaggedItem, GenericTaggedItemBase
from core.taggit.utils import require_instance_manager
...
class TaggableManager(RelatedField):
    ...
    def extra_filters(self, pieces, pos, negate):
        ...
        cts = map(
            ContentType.objects.get_for_model, _get_subclasses(self.model))
        if len(cts) == 1:
...
class _TaggableManager(models.Manager):
    ...
    def similar_objects(self):
        ...
        qset = self.through.objects.values(*lookup_kwargs.keys())
        qset = qset.annotate(n=models.Count('pk'))
        qset = qset.exclude(**lookup_kwargs)
        qset = qset.filter(tag__in=self.all())
        qset = qset.order_by('-n')
        ...
        if len(lookup_keys) == 1:
            ...
            # somehow?
            fld = self.through._meta.get_field_by_name(lookup_keys[0])[0]
            objs = fld.rel.to._default_manager.filter(**{
                "%s__in" % fld.rel.field_name: [r["content_object"] for r in qset]
            })
            for obj in objs:
                items[(getattr(obj, fld.rel.field_name),)] = obj
        else:
            preload = {}
            for result in qset:
            ...
            for ctype, obj_ids in preload.iteritems():
                ctype = ContentType.objects.get_for_id(ctype)
                for obj in ctype.model_class()._default_manager.\
                           filter(pk__in=obj_ids):
                    items[(ctype.pk, obj.pk)] = obj
...
def _get_subclasses(model):
    subclasses = [model]
    for fld in model._meta.get_all_field_names():
        field = model._meta.get_field_by_name(fld)[0]
